"""Groq LLM adapter. Uses OpenAI SDK with Groq API endpoint."""
...
class _ToolCallCollector:
    ...
    def feed_delta(self, tool_call_delta: Any) -> None:
        """Process one tool-call delta from a Groq streamed chunk."""
...
class GroqLLMProvider:
    def __init__(self, *, model: str, api_key: str, client: AsyncOpenAI | None = None) -> None:
        ...
        self._model = model
        self._client = client or AsyncOpenAI(api_key=api_key, base_url=_GROQ_BASE_URL)

    @property
    def provider_key(self) -> str:
        return _GROQ_PROVIDER_KEY
    ...
    async def stream(
        ...
        *,
        ...
        if request.tools:
            kwargs["tools"] = _to_groq_tools(request)
        ...
        try:
            ...
            raise _normalize_groq_error(exc) from exc


def _to_groq_messages(request: LLMRequest) -> list[dict[str, str]]:
...
def _to_groq_tools(request: LLMRequest) -> list[dict[str, Any]]:
    """Translate provider-neutral LLMToolDefinition to OpenAI-compatible tool format."""
...
def _normalize_groq_error(exc: Exception) -> ProviderError:
    if isinstance(exc, AuthenticationError):
        return ProviderError(
            code=ProviderErrorCode.AUTH,
            message="authentication failed",
            provider_key=_GROQ_PROVIDER_KEY,
            retryable=False,
        )
...
def usage_from_groq(usage: Any) -> Usage | None:
    if usage is None:
        return None
